...
from src.parsing.github import GithubParser
...
async def schedule_parsing() -> None:
	try:
		gh_parser = GithubParser(settings.API_KEY)
		
		new_top_repos = await gh_parser.parsing_top_rest_api()
		
		new_repo_activities = await gh_parser.parsing_activities_rest_api(
			list_of_repos=new_top_repos,
		)
